...
from confiture_pggit.coordination import Intent, IntentStatus, RiskLevel
...
def _run(command: str, *extra: str):
    intent_id = str(uuid4())
    with (
        patch("confiture_pggit.cli.coordinate._get_connection") as get_conn,
        patch("confiture_pggit.cli.coordinate.IntentRegistry") as registry_class,
    ):
