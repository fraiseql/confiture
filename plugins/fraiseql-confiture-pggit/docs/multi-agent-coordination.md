...
This guide covers the `confiture coordinate` CLI commands and multi-agent development patterns. The commands come from the `fraiseql-confiture-pggit` plugin; the [README](../README.md) shows how to install it.
...
- Multiple people or agents are working on the same database
- Schema changes are happening in parallel
- You want a conflict check before implementing changes
- You need an audit trail of who is working on what

**Skip coordination when:**
- You are a solo developer with full context
- Schema changes are serialized anyway (no parallel work)
...
The commands store intents and conflicts in a PostgreSQL database, named by
`--database-url` or by the `DATABASE_URL` / `CONFITURE_DB_URL` environment
variable. The coordination tables (`tb_pggit_intent`, `tb_pggit_conflict`,
`tb_pggit_intent_history`) are created there on first use; there is no separate
init step.

```bash
createdb confiture_coordination
export DATABASE_URL=postgresql://localhost/confiture_coordination
```
...
```bash
# Say you have started (optional, but it is what `in_progress` means)
...
# When they reach the main line. `merged` is a separate status from
...
not say why is not worth recording. The [command reference](cli.md#confiture-coordinate)
lists every option.
...
# 4. Mark as completed
confiture coordinate complete --intent-id int_solo123
```
...
- Mark `IN_PROGRESS` when starting work (`coordinate start`)
- Mark `COMPLETED` when done (`coordinate complete`), `MERGED` when it lands (`coordinate merge`)
- Mark `ABANDONED` if cancelled (`coordinate abandon`)
...
# Option 2: Set environment variable (CONFITURE_DB_URL works too)
...
Every `confiture coordinate` command takes `--format json`, so a pipeline can
check a pull request's schema changes against the registered intents and record
the merge when it lands. [Coordination in CI/CD](ci-integration.md) has GitHub
...
Multi-agent coordination is designed to work with this plugin's pgGit branching commands:
...
See the [`confiture branch` commands](cli.md#confiture-branch) for details.
...
```python
from confiture_pggit.coordination import IntentRegistry
import psycopg
...
- [Coordination in CI/CD](ci-integration.md)
- [How coordination works](multi-agent-coordination-architecture.md)
- [Performance benchmarks](coordination-performance.md)
- [Migration Hooks](../../../docs/guides/hooks.md) (confiture)
- [Schema Linting](../../../docs/guides/schema-linting.md) (confiture)
