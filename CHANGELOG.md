...
- **`confiture drift` and `migrate validate --check-live-drift` no longer report an
  ...
  so `id INTEGER GENERATED BY DEFAULT AS IDENTITY, …, CONSTRAINT pk PRIMARY KEY (id)`
  expected a nullable `id` and PostgreSQL — correctly — reported it `NOT NULL`: three
  `nullable_mismatch` warnings on a database applied verbatim from
  `examples/02-fraiseql-integration`, recorded in its drift golden and now gone
  from it. The lint inventory reads a table's constraints through the one
...
schema in its identity. The lint inventory keys `(kind, schema, name)`;
`ddl_objects.ObjectRef` keys the same way; `drift.py` qualifies both sides. The
differ keyed a bare `t.name`, and discarded `stmt.relation.schemaname` at parse
time. So inside one `compare()` call, `a.v` and `b.v` were two views — they live
in `ParsedSchema.objects`, which #288 built on `ObjectRef` — and `a.t` and `b.t`
were one table. The same run answered the same question two ways.

Reported against a 3.96 MB / 707-file schema with four colliding names
(`tb_meter`, `tb_reading`, `tb_attachment`, `tb_email_received`, each in
`tenant.` and `etl_ingest.`): 433 tables parsed, 429 distinct names, **four
tables permanently invisible to `migrate validate --require-migration`** — the
project's central fact tables.

It was not only silent. Swapping two schema files' build order — a rename, a
renumber, **no schema change at all** — made the differ compare `tenant.t`
against `etl.t` and report `DROP COLUMN t.b`, which `migrate diff --generate`
writes into a migration as `ALTER TABLE t DROP COLUMN IF EXISTS b`. It also
corrupted the parse itself: `ALTER TABLE etl.t ADD COLUMN` and
`CREATE INDEX … ON etl.t` landed on `tenant.t`, while `DROP TABLE etl.t` and
`ALTER TABLE etl.t RENAME TO` hit **both** tables.

And it shipped in this repository's own `examples/06-prep-seed-validation`,
which has a `run.sh` and runs in CI: the prep-seed pattern *is* two schemas
holding the same table names.

### ⚠️ Behaviour changes

- **A project with colliding table names starts seeing changes it has never
  seen.** That is the point, and it means **the first
  `migrate validate --require-migration` run after upgrading may fail on changes
  made months ago** — real changes, genuinely never carried by a migration,
  invisible to the gate when they were made. This is not a new false positive;
  it is the backlog the old gate accumulated.

  The remedy is to write the missing migrations, or to baseline. Rolling back to
  1.12.0 restores a gate that is green and wrong.

- **Generated DDL carries the schema qualifier the schema file wrote.** A
  migration generated by 1.13.0 differs textually from one generated by 1.12.0
  for the same diff: `ALTER TABLE tenant.t ADD COLUMN …` where 1.12.0 wrote
  `ALTER TABLE t`, and `REFERENCES b.parent(id)` where it wrote
  `REFERENCES parent(id)`. Both of the old forms resolve through `search_path`
  at apply time, which is not necessarily where the change was detected.

  **An unqualified schema tree generates byte-identical DDL to 1.12.0.**
  Confiture never invents a `public.` qualifier the file did not write: a project
  whose `search_path` is not `public` would otherwise have its DDL rewritten into
  another schema. Identity folds a missing qualifier; spelling never does.

- **`migrate diff --format json` gains a required `warnings` key**, the shape
  1.6.0 gave `build` (`code`, `severity`, `message`, `file`). It is present and
  empty when there is nothing to report, never absent-on-success. A consumer
  pinned to a pre-1.13.0 schema must update; `migrate-diff.schema.json` sets
  `additionalProperties: false`.

- **A duplicate definition is warned, not failed** (`DIFFER_402`). Two
  definitions of one `(schema, name)` in one tree is a real problem, but it is
  `confiture lint`'s problem (`build_001`) and `build --fail-on-duplicates`'
  problem, both of which already exist and are opt-in. Failing
  `--require-migration` for it would fail the gate for a reason the gate is not
  about — and would fail it for projects that have lived with the duplicate for
  years. Unlike #288's unparseable schema, the check here *ran* and produced a
  result; what is uncertain is whether the tree it read is the tree the build
  produces.

- **A generated index keeps the name the schema file gave it.**
  `_compare_indexes` wrote `details["index_name"]` and every reader in
  `differ_sql` asked for `details["name"]`, so every read took a fallback that
  fabricated `idx_{table}`. Consequences, all of them silent: an added index was
  created under a name nobody chose (and `confiture drift` then reported the
  divergence from the rebuild path forever); a second index on one table
  collided with the first and its `CREATE INDEX … IF NOT EXISTS` was a no-op;
  a dropped index generated no DDL at all; and `unique` never reached the
  generated statement, so `CREATE UNIQUE INDEX` came out non-unique.
  `str(change)` read the key correctly, which is why `migrate diff` *printed*
  `ADD INDEX ix` while generating `idx_t` — the report and the artefact
  disagreed, and only the artefact is applied.

  If you have a migration generated by ≤1.12.0 that created an `idx_<table>`
  index, that index is still there under that name; nothing in this release
  renames it.

- **No generated name is fabricated any more.** A change carrying no object name
  now emits the generator's existing `-- WARNING:` comment instead of
  `idx_{table}` / `fk_{table}` / `chk_{table}` / `uq_{table}`. A name confiture
  invented is indistinguishable from one the author chose, which is how the key
  mismatch above survived; and with a qualified table it is not even a legal
  identifier (`idx_tenant.t`).

### Fixed

- **`parse_schema` describes the files it read.** `Table` carries `schema`;
  `_table_named`, `_named`, `_drop_declared`, `_rename_declared` and
  `_rename_column` match on `(schema, name)` with the wildcard
  `ddl_objects._matches` already applied — a statement that wrote no schema did
  not say which one it meant. `_named` also checks the object *kind*, so
  `ALTER SEQUENCE s RENAME TO s2` no longer renames a table called `s`, and
  `DROP INDEX` is scoped to the schema it names.

- **`ALTER TABLE … SET SCHEMA` folds.** The docstring that stated the gap as a
  decision — *"a `Table` here is keyed by its bare name and carries no schema at
  all"* — is rewritten, that reason having expired.

- **Enum types and sequences use the schema they already carried.**
  `EnumType.schema` and `Sequence.schema` have existed since those models were
  written; `_compare_enum_types` and `_compare_sequences` keyed on `.name` and
  threw them away. A value added to `a.status` while `b.status` was present
  reported nothing; so did dropping `b.s1` while `a.s1` was present.

- **A rename is matched within one schema.** Not a threshold: `_similarity_score`
  scores `tenant.tb_meter` against `etl.tb_meter` at 0.6, which is exactly what
  it scores the real rename `tenant.tb_a` → `tenant.tb_b`. The separation is
  grammatical — `ALTER TABLE a.t RENAME TO a.t2` is a syntax error, and the
  operation that moves a table between schemas is `SET SCHEMA` — so a
  cross-schema pairing is a `DROP_TABLE` and an `ADD_TABLE`, which is what it is.

- **`RENAME_TABLE` generates SQL PostgreSQL accepts** in both directions. The
  change carries both qualified spellings and both bare names, so neither the
  `.sql` pair nor the `.py` migration body has to take a qualifier apart.

- **A collapse is a finding.** Two definitions of one `(schema, name)` in one
  tree are resolved by `duplicates.wins` — `build_001`'s own rule, imported
  rather than restated — so the diff reads the tree the build produces. A later
  `IF NOT EXISTS` is a no-op, so the **first** definition is what the database
  has; the differ used to keep the last, unconditionally, and compare a table the
  database does not have. `wins == "conflict"` says the build would fail at that
  statement.

### Added

- **`DIFFER_402`** (severity `warning`, exit 0) — an object defined more than
  once in one schema tree: the identity, the count, what the build does with it,
  and which definition the comparison used. It reaches
  `migrate diff --format json`'s `warnings[]` and
  `migrate validate --require-migration`'s report.

- **`core/schema_identity.py`** — `DEFAULT_SCHEMA`, moved out of
  `core/linting/inventory.py`, which decides object identity but also imports
  pglast. A sweep found the literal `"public"` spelled beside a schema variable
  at **nine sites in four modules**; two of those four need the fold and not a
  parser, which is why they wrote the word. All nine now import it; `inventory`
  re-exports it.

- **`Table.qualified` / `EnumType.qualified` / `Sequence.qualified`** and
  `models.schema.qualified_name` — the object as the schema file names it. The
  spelling a finding prints, never the identity that pairs two statements.

- **`tests/unit/test_one_object_identity.py`** — the guard. #313 is the fourth
  instance of one pattern here, and the first three each ended in a guard (#275,
  #256, #301). Prose did not hold: `ddl_objects.py` already said *"Identity is
  the inventory's answer, not a second one"*, and `differ.py` disagreed with it
  for as long as both existed. Two checks — no second default-schema fold, no
  schema-object model keyed by a bare name — both allow-lists empty, and an entry
  matching nothing fails. The check is pinned against `3b12dcce`'s three maps
  verbatim: a guard never seen red is a guard that does not run.

### Removed

- **`models.schema.Schema`** — an orphan. No production module imported or
  constructed it; it was kept alive by its own eight tests, and its
  `get_table` / `has_table` / `table_names` were all keyed by bare name — a
  third place to get #313 wrong, in code nothing ran. (The `Schema` in
  `tests/unit/hooks/` is a different class and is untouched.)

- Three empty comment banners and `_DDL_PREFIXES` in `core/differ.py`, left
  behind by the sqlparse removal D13 made unreachable.

### Known, and filed separately

- `ADD_CHECK_CONSTRAINT` still generates DDL that does not parse: the expression
  is stored as the AST *node type name* (`CHECK (A_Expr) ()`), because the
  comparison only ever needed identity and the generator was never told. It
  pre-dates #313 and is named with its reason in the parse sweep's
  `NOT_YET_PARSEABLE`.

- `CREATE TABLE a.child (pid INT REFERENCES b.parent(id))` parses to **zero**
  foreign keys — `_parse_column_pglast` reads `NOT NULL`, `PRIMARY KEY` and
  `DEFAULT` and ignores `CONSTR_FOREIGN`. Table-level `CONSTRAINT … FOREIGN KEY`
  is read correctly.

- `prep_seed/orchestrator.py` keys `TableDefinition` by bare name and separates
  prep from catalog by a path heuristic rather than by the qualifier `Table`
  now carries. It is named in the guard's `KNOWN_LATENT`, with an assertion that
  the shape is still there so the note cannot go stale.

[#313]: https://github.com/fraiseql/confiture/issues/313
