...
A spec that removes a table or a column, or narrows a type, asks for DDL that loses data. The
...
included); a dropped table comes back as its `CREATE TABLE`; a narrowed type widens back. Rows never
come back: the up statement that loses them is preceded by `-- confiture:irreversible data`, and
