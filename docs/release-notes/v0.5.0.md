...
**Documentation**: [Migration Hooks Guide](../guides/hooks.md)
...
**Documentation**: [Custom Anonymization Guide](../guides/anonymization.md)

---

### 3. Interactive Migration Wizard

> **Erratum (2026-09-17): this feature was never implemented.** At the `v0.5.0`
> tag, "wizard" appears in five files, all of them documentation — no Python
> source in that release, or any release before or since, contains it. The
> transcript below was never produced by a program. The two guides this section
> linked to were deleted on 2026-09-17; the section is kept, corrected, because
> a release note is a record and the record is that this was announced.
>
> What exists instead: `confiture migrate up --dry-run` analyses without
> executing, `--dry-run-execute` runs inside a SAVEPOINT that is always rolled
> back and prompts before applying for real (`--yes` skips that prompt), and
> `confiture migrate preflight --against <url>` replays onto a parallel database
> and reports the structural diff.
...
**Documentation**: none — see the erratum above.
...
⚠️  WARN: users.sql:12 - Missing index on 'email' column
❌ FAIL: users.sql:8 - Email column not encrypted
...
1. **[Migration Hooks](../guides/hooks.md)** (400 lines)
...
2. **[Custom Anonymization Strategies](../guides/anonymization.md)** (450 lines)
...
3. **Interactive Migration Wizard** (400 lines, deleted 2026-09-17 — never implemented)
...
5. **[Hooks vs Pre-commit](../guides/hooks.md)** (300 lines)
...
> **Erratum (2026-09-17): none of these four module paths exists, and two of
> the APIs never existed at all.** The hook and anonymization *functions* are
> real but live under `confiture.core.hooks` and `confiture.core.anonymization`
> (`register_strategy` is also re-exported from `confiture`); the hook API is
> `Hook[ExecutionContext]` + `Migrator.register_hook(HookPhase.X, hook)`, not a
> `@register_hook` decorator over plain functions. `confiture.linting` and
> `confiture.wizard` were never written: there is no `Rule` base class for users
> to subclass, no `RuleContext`, no `Violation`, no `register_rule`, and no
> wizard. Confiture's lint rules are a fixed registry
> (`confiture/core/linting/rule_registry.py`) selected with `--select` /
> `--ignore`; adding one is a change to confiture, not a plug-in.
>
> The reference pages built on this were deleted on 2026-09-17. The block is
> kept, corrected, because a release note is a record of what was announced.
...
**Results**:
```
✅ test_hooks.py              20/20 passed (100%)
✅ test_anonymization.py      22/22 passed (100%)
✅ test_linting.py            24/24 passed (100%)
✅ test_wizard.py             18/18 passed (100%)
────────────────────────────────────────────────────────────
✅ Phase 4 TOTAL             84/84 passed (100%)
```
...
- Cross-database migrations (PostgreSQL → MySQL, etc.)
...
1. **[Basic Migration](https://github.com/fraiseql/confiture/tree/main/examples/01-basic-migration)** - Added hooks example
2. **[FraiseQL Integration](https://github.com/fraiseql/confiture/tree/main/examples/02-fraiseql-integration)** - Added linting rules
3. **[Zero-Downtime Migration](https://github.com/fraiseql/confiture/tree/main/examples/03-zero-downtime-migration)** - Added custom anonymization
4. **[Production Sync](https://github.com/fraiseql/confiture/tree/main/examples/04-production-sync-anonymization)** - Enhanced anonymization strategies
5. **[Multi-Environment](https://github.com/fraiseql/confiture/tree/main/examples/05-multi-environment-workflow)** - Added wizard workflow
...
- **Phase 4a**: Hooks infrastructure (RED → GREEN → REFACTOR)
- **Phase 4b**: Anonymization strategies (RED → GREEN → REFACTOR)
- **Phase 4c**: Interactive wizard (RED → GREEN → REFACTOR)
- **Phase 4d**: Schema linting (RED → GREEN → REFACTOR)
- **Phase 4e**: Documentation & QA (RED → GREEN → REFACTOR → QA)
...
- **Migration Hooks**: [hooks.md](../guides/hooks.md)
- **Custom Anonymization**: [custom-anonymization-strategies.md](../guides/anonymization.md)
