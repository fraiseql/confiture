...
| Surface | Before 0.39.0 | 0.39.0 |
|---|---|---|
| `migrate preflight --against` | **queried** the literal `tb_confiture`, raising `UndefinedTable` on a renamed ledger | queries the configured table; presence now goes through `core.ledger` |
| `migrate status` (ledger-absent warning, text + JSON) | named `tb_confiture` | names the configured table |
| `migrate up --auto-detect-baseline` | named `tb_confiture` | names the configured table |
| `migrate introspect` (text + JSON) | printed `tb_confiture`, emitted `tb_confiture_present` | prints the configured table, emits `ledger_present` (see below) |
| `migrate reinit` **confirmation prompt** | named `tb_confiture` | names the configured table |
