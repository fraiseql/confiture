...
The built file is a fixed 14-line header, then the concatenated files:
...
--
-- Environment: local
-- Generated: 2026-09-26T13:42:06.232889
-- Body SHA-256: 23d5909b3142015e72e922388e34985059461d147ba30874326e3818c7315a28
-- Files Included: 3
--
...
-- ... every selected file, in build order, under its separator ...
```

`Body SHA-256` is the SHA-256 of everything after the header, so the same body
gives the same line whatever confiture version wrote it, and anyone can check it:

```bash
tail -n +15 db/generated/schema_local.sql | sha256sum
```

### Reproducible builds

`Generated:` is the only line that moves between two builds of one tree. Set
[`SOURCE_DATE_EPOCH`](https://reproducible-builds.org/docs/source-date-epoch/)
and it does not: the two files are byte-identical, and a CI step that checks the
committed bundle against its sources is a plain `diff`:

```bash
SOURCE_DATE_EPOCH=0 confiture build --env local --output /tmp/built.sql
diff /tmp/built.sql db/generated/schema_local.sql
```

(Build the committed bundle with the same `SOURCE_DATE_EPOCH`.)

### Performance

- **Speed**: <1 second for 1000+ files
- **Deterministic**: the same tree gives the same body, in the same order; with
  `SOURCE_DATE_EPOCH` set, the same bytes
- **Cacheable**: `--show-hash` prints the *source* fingerprint (the files and their
  paths) that template and artifact caches key on
...
| `--database-url` | `-d` | str | - | Database connection URL (required for --sequential, default: from config) |
...
| `--database-url` | `-d` | str | - | PG server URL. |
...
| `--database-url` | `-d` | str | - | PG server URL (default: from env config). |
| `--format` | `-f` | str | `text` | Output format: text or json (default: text) |
...
| `--missing-is-drift` | - | Flag | off | Treat a routine the source declares and the database has not got as critical drift (exit 1). Requires --check-signatures. Off by default: before a deploy an undeployed routine is what is about to be applied, and only the caller knows which question it is asking. |
...
|---|---|---|---|---|
| `--migrations-dir` | - | path | `db/migrations` | Migrations directory (default: db/migrations) |
| `--config` | `-c` | path | - | Configuration file (optional) |
| `--database-url` | `-d` | str | - | PostgreSQL DSN for the tracking database. Always wins over --config / --env and the env vars. The canonical CONFITURE_DATABASE_URL beats a *default* --config but conflicts with an *explicit* one (CONFIG_007); the ambient DATABASE_URL never overrides a present config. Pass --no-config to make the environment the sole source. When a DSN is supplied, no YAML is required (tracking table defaults to tb_confiture). SSH-tunnel configs still require --config. |
| `--no-config` | - | Flag | off | Suppress config-file discovery entirely; the environment (CONFITURE_DATABASE_URL, else DATABASE_URL) becomes the sole DSN source. Use this for runtime-resolved DSNs that must not be exposed in argv. |
...
| `--check-tenant-isolation` | - | Flag | off | Deprecated alias for `--select default,tenant`: the tenant family (tenant_001–tenant_005), which a `tenancy:` block in db/project.yaml already turns on. |
...
| `--seed-env` | - | str | `development` | Seed directory under db/seeds/ to write into (default: development) |
...
| `--database-url` | `-d` | str | - | Database URL for database mode validation (default: none) |
| `--format` | `-f` | str | `text` | Output format: text or json or csv (default: text) |
| `--output` | `-o` | path | - | Write the output to this file instead of stdout |
