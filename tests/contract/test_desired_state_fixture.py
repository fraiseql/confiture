...
wrote for ``types.json``; ``expected_migration.py`` is the migration confiture
...
def test_artifact_generates_the_checked_in_migration(tmp_path: Path) -> None:
    ...
    generated = MigrationGenerator(migrations_dir=tmp_path).generate(
        diff, name="init_from_spec", version="20260101000000"
    )

    expected = (FIXTURES / "expected_migration.py").read_text()
