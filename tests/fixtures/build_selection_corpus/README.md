...
- `diagnostics` — the `CONFIG_013` / `CONFIG_014` notes the shape reports, if any.
...
| `non_recursive_depth_pattern` | `recursive: false` + `**/sub/*.sql` | ✅ selects nothing, and says why |
| `slash_free_exclude` | `exclude: ["*.bak"]` | ❌ control — a `/`-free pattern cannot have changed |

Note that four of the changed shapes report **no diagnostic**: `order`, deduplication and overlap
resolution are not glob semantics, so no `CONFIG_013`/`CONFIG_014` names them. `confiture build
--list-files` is the tool for those.
