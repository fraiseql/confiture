...
def test_ignore_glob_star_schema(own_db: psycopg.Connection) -> None:
    ...
    with own_db.cursor() as cur:
        ...
        cur.execute(f"ALTER TABLE {_TEST_SCHEMA}.legacy_audit_log OWNER TO own_intruder")
    own_db.commit()
...
def test_materialized_view_owned_by_wrong_role_flagged(own_db: psycopg.Connection) -> None:
    with own_db.cursor() as cur:
        ...
        cur.execute(f"ALTER MATERIALIZED VIEW {_TEST_SCHEMA}.mv_drift OWNER TO own_intruder")
    own_db.commit()
...
def test_partition_child_with_drifted_owner_is_flagged(own_db: psycopg.Connection) -> None:
    with own_db.cursor() as cur:
        ...
        cur.execute(f"ALTER TABLE {_TEST_SCHEMA}.tb_events_2026 OWNER TO own_intruder")
    own_db.commit()
