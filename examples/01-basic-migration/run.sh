...
# A fresh database is built from db/schema — the single source of truth, which
# already carries the effect of every migration. The ledger is then baselined so
# `migrate status` agrees; `migrate up` is for databases that predate 001.
schema="$(mktemp)"
...
psql "$DB_URL" -v ON_ERROR_STOP=1 -Atc "SELECT column_name FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'bio'" | grep -qx bio

echo "✅ 01-basic-migration: ok"
