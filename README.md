...
- JSON schemas are published for the `--format json` output of `build`, `drift`, `introspect`, `lint`, `lint-unified`, `schema dump-model`, `sync`, `validate-config`, `verify-checksums` and, in the migrate family, `migrate up`, `migrate down`, `migrate down-to`, `migrate rebuild`, `migrate reinit`, `migrate status`, `migrate current`, `migrate diff`, `migrate fix`, `migrate introspect`, `migrate preflight`, `migrate steps`, `migrate validate` and `migrate verify`. They ship in the package (`python/confiture/schemas/`) and are mirrored under [`docs/reference/json-schemas/`](docs/reference/json-schemas/); a test asserts the mirror equals the packaged source (see [`docs/reference/json-schemas.md`](docs/reference/json-schemas.md)). The other commands' JSON payloads have no published schema yet; `tests/unit/json_schemas/test_every_json_command_has_a_schema.py` lists them, and that list only shrinks.
