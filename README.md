...
- JSON schemas are published for the `--format json` output of all but four commands — `debug cte`, `migrate fix-signatures`, `migrate generate` and `seed validate`, each held back by a defect in its output that `tests/unit/json_schemas/test_every_json_command_has_a_schema.py` names. They ship in the package (`python/confiture/schemas/`) and are mirrored under [`docs/reference/json-schemas/`](docs/reference/json-schemas/), one section per command in [`docs/reference/json-schemas.md`](docs/reference/json-schemas.md); a test asserts the mirror equals the packaged source, and each schema is validated against payloads the command wrote.
