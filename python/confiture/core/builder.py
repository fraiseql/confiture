...
class SchemaBuilder:
    ...
    def build(
        ...
        schema = self._build_python("", files, progress=progress)
        ...
        if self.env_config.build.two_pass:
            ...
            if fk_infos:
                ...
                schema = stripped_sql
        schema = self._with_header(schema, len(files))
    ...
    def build_split(
        ...
        if superuser_pre_files:
            superuser_pre_sql = self._with_header(
                self._build_python("", superuser_pre_files), len(superuser_pre_files)
            )
        else:
            superuser_pre_sql = ""
        ...
        # Build app SQL
        app_sql = self._build_python("", app_files)
        ...
        if self.env_config.build.two_pass:
            ...
            if fk_infos:
                ...
                app_sql = stripped_sql
        app_sql = self._with_header(app_sql, len(app_files))

        app_path.write_text(app_sql, encoding="utf-8")

        # Build superuser post SQL
        if superuser_post_files:
            superuser_post_sql = self._with_header(
                self._build_python("", superuser_post_files), len(superuser_post_files)
            )
        else:
            superuser_post_sql = ""
    ...
    def _with_header(self, body: str, file_count: int) -> str:
        """``body`` under the header that describes it.

        The header says only what it can prove about this file: the SHA-256 of
        the body below it — the same body gives the same line whatever confiture
        wrote it — and a timestamp taken from ``SOURCE_DATE_EPOCH`` when set, so
        two builds of one tree are byte-identical and compare with a plain
        ``diff`` (#428). The source fingerprint, :meth:`compute_hash`, keys
        caches and is reported by ``build --show-hash``, not here.
        """
        epoch = os.environ.get("SOURCE_DATE_EPOCH")
        moment = datetime.fromtimestamp(int(epoch), UTC) if epoch else datetime.now()
