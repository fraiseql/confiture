...
_RE_CREATE_VIEW = re.compile(rf"CREATE\s+(?:OR\s+REPLACE\s+)?VIEW\s+{_QUAL}", re.IGNORECASE)
_RE_ALTER_TABLE = re.compile(rf"ALTER\s+TABLE\s+(?:ONLY\s+)?{_QUAL}", re.IGNORECASE)
_RE_ALTER_VIEW = re.compile(rf"ALTER\s+VIEW\s+{_QUAL}", re.IGNORECASE)
_RE_ALTER_MATVIEW = re.compile(rf"ALTER\s+MATERIALIZED\s+VIEW\s+{_QUAL}", re.IGNORECASE)
...
_RE_RENAME_COLUMN = re.compile(rf"RENAME\s+(?:COLUMN\s+)?({_DENT})\s+TO\s+({_DENT})", re.IGNORECASE)
